...
# Usar fuente por defecto si no hay fuentes TTF
font_title = ImageFont.load_default()
font_subtitle = ImageFont.load_default()

# Calcular tamaño de texto
# Para títulos grandes, usar texto más simple
draw.text((50, 50), 'EL TERREMOTO TECNOLÓGICO', fill=(255, 255, 255), font=font_title)
draw.text((50, 110), 'QUE NADIE VIÓ VENIR', fill=(255, 255, 255), font=font_title)

# Añadir subtítulo
# Usar texto más simple para subtítulos
draw.text((50, 180), '20 de Febrero, 2026 - El día que Wall Street entró en pánico', fill=(200, 200, 200), font=font_subtitle)
...
for i, (company, change, color) in enumerate(stock_data):
    ...
    draw.text((x, y_start + max_height + 10), company, fill=(255, 255, 255), font=font_subtitle)
    draw.text((x, y_start + max_height - bar_height - 30), f'{change}%', fill=(255, 255, 255), font=font_subtitle)
...
for i, (title, description, color) in enumerate(highlights):
    draw.text((50, y_highlight + i * 40), f'• {title}: {description}', fill=color, font=font_subtitle)
...
# Hashtags finales
draw.text((50, height - 50), '#ClaudeCode #TechDisruption #Cybersecurity #Innovation', fill=(150, 150, 150), font=font_subtitle)
